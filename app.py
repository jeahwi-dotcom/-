...
st.caption("제시된 가중치 수식에 따른 정밀 매수 가이드")
st.divider()
...
# 4. 수식 기반 가중치 계산 함수 (사용자 요청 수식 100% 반영)
...
if data:
    ...
    st.markdown(f"""
        <div style="background-color: #1E1E1E; padding: 40px; border-radius: 20px; border: 3px solid #EAFF00; text-align: center;">
            ...
            <p style="color: #888; font-size: 1.1rem; margin-top: 20px;">
                합산 가중치 배수: <b>{total_multiplier:.1f}배</b> <br>
    ...
    # 상세 계산 근거
    with st.expander("🔍 상세 계산 수식 및 테이블 확인"):
        st.write(f"**1. VIX 가중치:** {v_w} (현재 {data['vix']:.2f})")
        st.write(f"**2. MDD 가중치:** {m_w} (현재 {data['mdd']:.2f}%)")
        st.write(f"**3. RSI 가중치:** {r_w} (현재 {data['rsi']:.2f})")
        st.write("---")
        st.write(f"**최종 계산:** {base_amount:,.0f}원 × ({v_w} + {m_w} + {r_w}) = {final_amount:,.0f}원")
else:
    st.warning("시장 데이터를 불러오는 데 실패했습니다. 잠시 후 다시 시도해주세요.")
