...
st.caption("공식: 오늘의 최종 매수액 = 기본 매수액 × (VIX 가중치 + MDD 가중치 + RSI 가중치) / 3")
st.divider()
...
if data:
    ...
    # [수정된 공식 적용] 
    # 오늘의 최종 매수액 = 기본 매수액 × (VIX + MDD + RSI) / 3
    avg_multiplier = (v_w + m_w + r_w) / 3
    final_amount = base_amount * avg_multiplier
    ...
    st.markdown(f"""
        <div style="background-color: #1E1E1E; padding: 40px; border-radius: 20px; border: 3px solid #EAFF00; text-align: center;">
            ...
            <p style="color: #888; font-size: 1.1rem; margin-top: 20px;">
                적용된 평균 가중치 배수: <b>{avg_multiplier:.2f}배</b><br>
                (VIX {v_w} + MDD {m_w} + RSI {r_w}) / 3
            </p>
        </div>
    ...
    with st.expander("🔍 가중치 산정 상세 근거 확인"):
        ...
        st.write("---")
        st.write(f"**최종 계산:** {base_amount:,.0f}원 × ({v_w} + {m_w} + {r_w}) / 3 = {final_amount:,.0f}원")
else:
