...
# 3. 실시간 데이터 로드 함수 (VIX 포함 웹 데이터 수집)
@st.cache_data(ttl=600)
def get_market_data():
    try:
        # 웹(Yahoo Finance)에서 실시간으로 받아올 티커 리스트
        tickers = {
            "spy": "^GSPC",           # S&P 500 시장 지수
            "vix": "^VIX",            # VIX (공포 지수) - 웹에서 실시간 수집
